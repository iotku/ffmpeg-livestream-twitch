...
ffmpeg -f v4l2 -video_size 640x480 -framerate 29.97 -thread_queue_size 50k -i /dev/video0 -thread_queue_size 50k \
    ...
    -filter_complex "[0:v]bwdif[deint];[deint]crop=w=${CROP_W}:h=${CROP_H}:x=${CROP_X}:y=${CROP_Y}[deint];[deint]scale=${CALC_CAP_W}:${STREAM_H}[deint];[1:v]scale=${WIDTH_LEFT}:${CALC_SPLIT_H}[splits];[splits]pad=${STREAM_W}:${STREAM_H}[splits];[splits][deint]overlay=${WIDTH_LEFT}:0[vid];[2:a]adelay=${AUDIO_DELAY}|${AUDIO_DELAY}[daudio]"\
    -map [vid] -map [daudio] \
    480poutput.mkv
