...
ffmpeg -f v4l2 -video_size 640x480 -framerate 29.97 -thread_queue_size 50k -i /dev/video0 -thread_queue_size 50k \
    ...
    -filter_complex "[0:v]bwdif[deint];[deint]crop=w=570:h=448:x=28[deint];[1:v]scale=220:320[splits];[splits]pad=786:448[splits];[splits][deint]overlay=220:0[vid];[2:a]adelay=335|335[daudio]"\
    -map [vid] -map [daudio]\
    -f tee "localrecord.mkv|[f=flv]rtmp://${TWITCH_STREAM_URL_IOTKU}"
